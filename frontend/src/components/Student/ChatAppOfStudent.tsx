...
import ChatBoxOfStudent from "./ChatBoxOfStudent.tsx";
...
export default function ChatAppOfStudent({instructorId}:{instructorId?:string|null}) {
    ...
    const fetchRooms = async () => {
      try {
        ...
        const res = await axios.post(
          `${import.meta.env.VITE_BACKEND_URL}/chats/messagesStudent`,
          values
        );
        ...
        if(instructorId){
          const targetRoom = (res.data.chatRooms ?? []).find(
             (room:ChatRoom) => room.participants.includes(instructorId)
         );
          console.log(targetRoom.id);
          if(instructorId && targetRoom) setActiveRoom(targetRoom.id); 
        }
      } catch (error) {
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50">
          {activeRoom ? (
            <ChatBoxOfStudent roomId={activeRoom} />
          ) : (
...
//             values
//           );
//           console.log("res",res.data.chatRooms)
//           if (res.data.success) {
//             setRooms(res.data.chatRooms ?? []);
//         }
...
//           room.participants.includes(instructorId))
//           console.log(targetRoom)
//           if(targetRoom) setActiveRoom(targetRoom.id);
//         }
//       } catch (error) {
//         console.error("Lỗi khi load danh sách phòng chat:", error);
//       }
